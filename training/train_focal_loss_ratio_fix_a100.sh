#!/bin/bash
#SBATCH --job-name=varunanet_focal_loss_ratio_fix
#SBATCH --output=%j.out
#SBATCH --error=%j.err
...
# for the full rationale). Original run: training/train_focal_loss_a100.sh, job 1789,
# 0.3359 mean IoU under the broken ratio channel + stale normalization stats.
