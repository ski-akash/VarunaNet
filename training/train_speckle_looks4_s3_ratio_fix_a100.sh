#!/bin/bash
#SBATCH --job-name=varunanet_speckle_looks4_s3_ratio_fix
#SBATCH --output=%j.out
#SBATCH --error=%j.err
...
# for the full rationale). Original run: training/train_speckle_looks4_s3_a100.sh, job 1757,
# 0.3977 mean IoU under the broken ratio channel + stale normalization stats.
...
python -m training.train dataset=sen1floods11 dataset.speckle_prob=0.5 dataset.speckle_looks=4.0 \
    device=cuda epochs=30 seed=3 early_stopping_patience=8 \
    wandb.mode=disabled \
