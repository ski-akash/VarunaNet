#!/bin/bash
#SBATCH --job-name=varunanet_speckle_looks10_ratio_fix
#SBATCH --output=%j.out
#SBATCH --error=%j.err
...
# for the full rationale). Original run: training/train_speckle_looks10_a100.sh, job 1733,
# 0.4106 mean IoU under the broken ratio channel + stale normalization stats.
# Same recipe, only the (already-fixed) data pipeline is different.
python -m training.train dataset=sen1floods11 dataset.speckle_prob=0.5 dataset.speckle_looks=10.0 \
